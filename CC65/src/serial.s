;
; Adapted from: Serial driver for the Apple II Super Serial Card.
;
...
; ---------------------------------------------------------------
; unsigned char __near__ _SerialOpen (void* ptr)
; ---------------------------------------------------------------
;
; A pointer to a ser_params structure is passed in ptr1.
...
:       ldy     IdOfsTable,x
        ...
        tay
        lda     ParityTable,y
        ora     #%00000001              ; DTR active (commented = Interrupt disabled)
        sta     RtsOff
...
; ---------------------------------------------------------------
;
; Will fetch a character from the receive buffer and store it into the variable 
; pointed to by ptr1. If no data is available, SER_ERR_NO_DATA is returned.

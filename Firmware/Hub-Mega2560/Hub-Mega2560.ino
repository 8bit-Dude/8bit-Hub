...
//#define __DEBUG_LIC__
//#define __DEBUG_PACKETS__
//#define __DEBUG_ESP__
...
#define HUB_FILES    10    // Number of file handles
#define HUB_CONNECTS 10    // Number of connection handles
...
// HUB Params
byte hub_mode = MODE_LYNX; 

// SD Card Paramds
File hubFile[HUB_FILES];
...
void pushPacket(unsigned char cmd, unsigned char* data, unsigned char len) {
  // Create new packet
    ...
    packet->ID = countID;
    packet->timeout = millis() + HUB_TIMEOUT;
...
void readInt(unsigned int *buffer) {
    ...
    Serial3.readBytes((char*)buffer, 2);  
}
...
void readUdp() {
    ...
    unsigned char data[256], len;
    len = readChar();
    Serial3.readBytes(data, len);
    pushPacket(HUB_UDP_RECV, data ,len);  
...
void readTcp() {
    ...
    len = readChar();
    Serial3.readBytes(data, len);
    pushPacket(HUB_TCP_RECV, data ,len);  
...
void readSrv() {
    ...
    len = readChar();
    Serial3.readBytes(data, len);
    pushPacket(HUB_SRV_RECV, data ,len);  
...
void setupESP() {
    ...
    while (millis() < timeout) {
        if (Serial3.find("CMD") && readChar() == HUB_ESP_NOTIF) {
            readNotif();
            break;
        }
    }        
...
unsigned char lynxRecvPacket() {
    ...
    // Get Buffer
    if (txLen > 0) {
        if (!Serial2.readBytes(txData, txLen)) { return COMM_ERR_TRUNCAT; }
    }
...
const char* menuParam[2][3] = {{"Mode:", "Test:", "Exit:"}, {"SSID:", "Pass:", ""}};
...
void configMenu() {
    ...
    while (true) {
        ...
        for (byte param=1; param<4; param++) {
            // Show Param
        ...
        // Check Input
        if (!(joyState[0] & joyStnd[0])) { row--; if (row<1) row = 2; }
        ...
        if (!(joyState[0] & joyStnd[4]) || !(joyState[0] & joyStnd[5])) {   
            ...
            case 2:
                ...
                case 1:
                    // Set SSID
                    softKeyboard(menuParam[menu-1][row-1]);
...
void loop() { 
    ...
    if (code != COMM_ERR_NODATA) {
        ...
        //Serial.print(count++);
        Serial.print("(Tx)");
        Serial.print(" Tim="); Serial.print(micros()-time1);
        ...
        if (txLen) {
            ...
            case HUB_SYS_RESET:
                ...
                for (byte i=0; i<HUB_FILES; i++) {
                    if (hubFile[i]) hubFile[i].close();
                }         
                countID = 0;
                rxID = 0;
            #ifdef __DEBUG_CMD__      
                Serial.println("HUB_SYS_RESET");
            #endif
                break;

            case HUB_FIL_OPEN:
                // Check if file was previously opened
                if (hubFile[txData[1]]) { hubFile[txData[1]].close(); }
                
                // Open file (modes are 0:read, 1:write, 2:append)
                switch (txData[2]) {
                case 0:                
                    hubFile[txData[1]] = SD.open(&txData[3], FILE_READ); 
                    break;
                case 1:
                    if (SD.exists(&txData[3])) { SD.remove(&txData[3]); }
                    hubFile[txData[1]] = SD.open(&txData[3], FILE_WRITE); 
                    break;
                case 2:                
                    hubFile[txData[1]] = SD.open(&txData[3], FILE_WRITE);
                    break;
            ...
            #ifdef __DEBUG_CMD__      
                Serial.print("HUB_FIL_OPEN #"); Serial.print((byte)txData[1]);
                Serial.print(": "); Serial.println((char*)&txData[3]);
            #endif                
                break;
            ...
            case HUB_FIL_READ:
                ...
                if (hubFile[txData[1]]) {
                    ...
                    while (hubFile[txData[1]].available() && tmp<txData[2]) {
                        buffer[tmp++] = hubFile[txData[1]].read();
                    }
                    pushPacket(HUB_FIL_READ, buffer, tmp);
                    free(buffer);
                }
                break;
            ...
            case HUB_FIL_WRITE:
                // Close file                
                if (hubFile[txData[1]]) hubFile[txData[1]].write(&txData[2], txLen-3);
                break;
            ...
            case HUB_FIL_CLOSE:
                ...
                if (hubFile[txData[1]]) hubFile[txData[1]].close();
            #ifdef __DEBUG_CMD__      
                Serial.print("HUB_FIL_CLOSE #"); Serial.println((byte)txData[1]);
            #endif
                break;                          
            ...
            case HUB_UDP_OPEN:
                ...
                writeCMD(HUB_UDP_OPEN);
                Serial3.write(txData[1]); Serial3.write(txData[2]);
                Serial3.write(txData[3]); Serial3.write(txData[4]);
                writeInt(txData[5]+256*txData[6]); 
                writeInt(txData[7]+256*txData[8]);
            #ifdef __DEBUG_CMD__      
                Serial.println("HUB_UDP_OPEN");
            #endif
                break;
